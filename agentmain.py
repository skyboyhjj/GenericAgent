...
import random
import locale
os.environ.setdefault('GA_LANG', 'zh' if any(
    k in (locale.getlocale()[0] or '').lower() for k in ('zh', 'chinese')) else 'en')
...
def load_tool_schema(suffix=''):
    ...
    TS = open(os.path.join(
        script_dir, f'assets/tools_schema{suffix}.json'), 'r', encoding='utf-8').read()
...
if not os.path.exists(mem_insight):
    ...
    open(mem_insight, 'w', encoding='utf-8').write(open(t,
                                                        encoding='utf-8').read() if os.path.exists(t) else '')
...
if not os.path.exists(cdp_cfg):
    try:
        ...
        open(cdp_cfg, 'w', encoding='utf-8').write(
...
def get_system_prompt():
    ...
    return inject_soul_prompt(prompt)
...
class GeneraticAgent:
        ...
        for k, cfg in mykeys.items():
            ...
            try:
                if 'native' in k and 'claude' in k:
                    llm_sessions += [NativeToolClient(
                        NativeClaudeSession(cfg=cfg))]
                elif 'native' in k and 'oai' in k:
                    llm_sessions += [NativeToolClient(
                        NativeOAISession(cfg=cfg))]
                elif 'claude' in k:
    ...
    def next_llm(self, n=-1):
        self.load_llm_sessions()
        self.llm_no = ((self.llm_no + 1) if n <
                       0 else n) % len(self.llmclients)
    ...
    def _handle_slash_cmd(self, raw_query, display_queue):
        ...
        if _sm := re.match(r'/session\.(\w+)=(.*)', raw_query.strip()):
            ...
            display_queue.put({'done': smart_format(
    ...
    def run(self):
        while True:
            ...
            raw_query, source, images, display_queue = task["query"], task["source"], task.get(
                "images") or [], task["output"]
            ...
            try:
                ...
                if '</file_content>' in full_resp:
                    full_resp = re.sub(r'<file_content>\s*(.*?)\s*</file_content>',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bg', action='store_true',
                        help='popen, print PID, exit')
    args = parser.parse_args()

    if args.bg:
        import subprocess
        import platform
        cmd = [sys.executable, os.path.abspath(
            __file__)] + [a for a in sys.argv[1:] if a != '--bg']
        d = os.path.join(script_dir, f'temp/{args.task}')
        os.makedirs(d, exist_ok=True)
        p = subprocess.Popen(cmd, cwd=script_dir,
                             creationflags=0x08000000 if platform.system() == 'Windows' else 0,
                             stdout=open(os.path.join(
                                 d, 'stdout.log'), 'w', encoding='utf-8'),
    ...
    if args.task:
        ...
        while True:
            ...
            open(os.path.join(log_dir, f'{script_name}_{datetime.now():%Y-%m-%d}.log'),
                 'a', encoding='utf-8').write(f'[{datetime.now():%m-%d %H:%M}]\n{result}\n\n')
